...
For LASSO-Markowitz (\S\ref{sec:lasso}) to recover a $k$-sparse support, Cand\`es and Tao's
condition \cite{candes-tao-rip} requires $\delta_{2k}(\hat\Sigma) < \sqrt 2 - 1\approx 0.414$,
where
\[
  \delta_s(\hat\Sigma) \;:=\; \sup_{|S|\le s}\,\big|\lambda_{\max}(\hat\Sigma_{S,S}) - 1\big| \;\vee\;
                              \big|\lambda_{\min}(\hat\Sigma_{S,S}) - 1\big|
\]
after re-scaling so that $\mathrm{diag}(\hat\Sigma) = 1$. We measure $\delta_k$ over $10^4$
random $k$-subsets (a cheaper Monte-Carlo estimate); since the supremum over $k$-subsets
is bounded by the supremum over $2k$-subsets, $\delta_k \le \delta_{2k}$ deterministically, so
any lower bound on $\delta_k$ certifies the same lower bound on $\delta_{2k}$.
The $95$-th-percentile values are
\[
  \delta_5 \approx 0.846,\qquad \delta_{10} \approx 0.867,\qquad \delta_{20} \approx 1.045,
  \qquad \delta_{50}\approx 0.853,
\]
each far exceeding $\sqrt 2 - 1$. Hence $\delta_{2k}\ge \delta_k > \sqrt 2 - 1$ at the
$95$-th percentile for every $k$ we test, certifying that the Cand\`es-Tao recovery
condition fails uniformly. We therefore expect LASSO-Markowitz to be a poor recovery
algorithm on this dataset, a prediction that Phase~2a confirms quantitatively.

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\textwidth]{figs/rip_probe.png}
\caption{Empirical RIP probe. For each $k\in\{5,10,20,50\}$ we evaluate $\delta_k$ on $10^4$ random subsets and show the median, $95$-th percentile, and maximum. Since $\delta_{2k}\ge\delta_k$ deterministically, any lower bound on $\delta_k$ is also a lower bound on $\delta_{2k}$. The dashed line is the Cand\`es-Tao threshold $\sqrt 2 - 1\approx 0.414$ that $\delta_{2k}$ must satisfy. The $95$-th percentile of $\delta_k$ exceeds the threshold for every $k$, certifying that $\delta_{2k}$ fails the condition; LASSO-Markowitz cannot be expected to recover the combinatorial optimum.}\label{fig:rip}
\end{figure}
