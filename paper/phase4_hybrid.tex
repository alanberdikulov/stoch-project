...
We design \textsc{Slh}\ (Spectral-Local Hybrid), a deterministic algorithm tailored
...
search that exploits the residual quadratic interaction in $J$. \cref{prop:slh-recovery}
gives an exact-recovery proposition under a factor-plus-residual model matching the
structure of the empirical covariance, and we benchmark \textsc{Slh}\ against the four
baselines of~\S\ref{sec:lasso}. We do not claim that \textsc{Slh}\ ``circumvents''
Corollary~\ref{cor:hardness}: as~\S\ref{sec:slh-regime} explains, the empirical SNR sits
deep in Regime~I (Table~\ref{tab:regime-numbers}), well below the regime where the
corollary binds, and the algorithm's success on real data is structural rather than
OGP-related.
...
We analyse \textsc{Slh}\ on a planted-spike instance with covariance matching the
structure documented in~\S\ref{sec:diagnostics}: a rank-$r$ deterministic factor part
plus an isotropic residual. A pure-isotropic $\hat\Sigma=\nu^2 I$ would make the
proposition tautological (top-$k$ of $\hat\mu$ already solves the problem); the factor
part is the substantive complication.
...
$\hat\Sigma=\nu^2 I$, the \textsc{Slh}\ seed would reduce to top-$k$ of $|\hat\mu|$,
and the proposition above would degenerate to the textbook ``top-$k$ recovers planted
support'' statement. The factor part $U\Lambda U^\top$ matters in two ways. It inflates
the variance of $\hat\mu_i$ for stocks loading on the market factor, which would
distort a raw top-$k$ of $\hat\mu$; and it is whitened out by the ridge solve
in~\eqref{eq:ridge-decomp}, so top-$k$ of $|w_{\mathrm{init}}|$ ranks stocks by
idiosyncratic return rather than by the factor-distorted $\hat\mu$. The proof shows
this whitening preserves exact recovery as long as the residual SNR exceeds
$a_{\mathrm{rec}}$. On real S\&P data Phase~1 documents $r\in[5,32]$ factor
eigenvalues and a residual variance $\nu^2 = (1/n)\mathrm{tr}(\hat\Sigma_\perp)$ that
...
There is a natural temptation to read \textsc{Slh}\ as ``escaping'' the hardness
corollary~\ref{cor:hardness}. We resist that reading, because it is only meaningful for
synthetic instances in Regime~II/III; on real S\&P data the corollary is vacuous for a
different and unrelated reason.
...
\paragraph{Why \textsc{Slh} recovers on synthetic Regime~III instances.}
Because Corollary~\ref{cor:hardness} is vacuous in Regime~III:
Theorem~\ref{prop:slh-recovery} forces $\mathcal A(\hat\mu_0) = \sigma^\star$ already
at $s = 0$, so the bridge never starts at overlap $\rho$ and the corollary's
hypothesis fails. The synthetic recovery curve of Fig.~\ref{fig:slh-recovery} shows
the predicted phase transition at $\hat a_{50\%} \approx 2.25$.

\paragraph{Why \textsc{Slh} recovers on real S\&P data.}
For a different and unrelated reason. By Table~\ref{tab:regime-numbers}, the empirical
SNR $\hat a \in [0.02, 0.05]$ sits deep in Regime~I, where the planted-spike model
predicts no recovery and is itself a poor fit (\S\ref{sec:regimes}). \textsc{Slh}'s
in-sample success on real data is not a Regime~III effect; it is explained by three
structural features of $(\hat\mu, \hat\Sigma)$ on S\&P~500: $C(n,k)$ is small (at most
$\sim 2\times 10^6$ on the brute-force cells), so the discrete optimisation has limited
combinatorial depth; $\hat\Sigma$ is dominated by a rank-one market factor
(\S\ref{ssec:spectrum}), making the marginal contribution of each asset nearly
separable so greedy-style methods are nearly optimal; and the quadratic coupling
$\gamma/k^2$ is small at $\gamma=10$, $k\in[5,10]$, so the residual interaction between
supports is weak and one polish pass suffices. None of these is an OGP statement.
\textsc{Slh}\ is the right algorithm for this dataset because it exploits the factor
structure (the seed) and then cleans up the small residual quadratic interaction (the
polish); Theorems~\ref{thm:detect} and~\ref{thm:forbid} are not what makes it work on
real data.

\paragraph{Where the OGP machinery is binding.}
On synthetic instances in Regime~II the corollary actively excludes \textsc{Slh}: on
the calibrated planted-spike instances of~\S\ref{ssec:p3b-synth}, \textsc{Slh}-style
top-$k$ thresholding fails for $a < 2.25$, while simulated annealing succeeds. The
synthetic recovery curve of Fig.~\ref{fig:slh-recovery} confirms this. On real S\&P
data we never enter Regime~II under the daily-return calibration, so the corollary's
exclusion does not directly translate into a real-data prediction; whether shrinkage
of $\hat\mu$ (\S\ref{ssec:ogp-discussion}) could move the problem into Regime~II is
the principal practical follow-up question raised by this analysis.

\subsection{Empirical performance}\label{sec:slh-empirical}

\textbf{Synthetic recovery curve.} On the planted-spike model $(n,k,\nu)=(200,40,1.0)$
over $50$ trials per SNR $a\in[0.4, 2.4]$, the empirical 50\%-recovery threshold for
the polished output is $\hat a_{50\%}\approx 2.25$, about $0.4$ below the Bonferroni
prediction $a_{\mathrm{rec}}=2.64$. This gap is the Sudakov-Fernique correction
...
curve sits strictly above the seed-only curve at every sub-threshold $a$
(\cref{fig:slh-recovery}), evidence that the polish stage adds recovery probability
the spectral seed cannot reach on its own.
...
\textbf{Real S\&P~500 benchmark.} On the same $(n,k)$ grid as~\S\ref{sec:oracle} with
five random instances per cell ($\gamma=10$), \textsc{Slh}\ matches the brute-force
oracle on every cell (mean optimality gap $\le 10^{-3}$\,bp, oracle-match rate
$\ge 0.8$ on $(15,4)$ and $1.0$ elsewhere), strictly outperforming greedy-forward (gaps
$0.01$ to $0.12$\,bp), simulated annealing (matches on $(10,3)$ through $(20,5)$,
degrades to $0.4$\,bp on $(50,5)$), LASSO ($2$ to $6$\,bp), and top-eigenvector ($11$
to $22$\,bp). The spectral seed alone (\textsc{Slh}-seed) gives gaps of $2$ to
$4$\,bp, comparable to LASSO; the polish stage closes essentially the entire remaining
gap (Figure~\ref{fig:slh-real-benchmark}). The split isolates the contribution of the
local-improvement step on real data and confirms the design intuition: the spectral
seed picks up the dominant rank-one factor structure of~\S\ref{sec:diagnostics}, and
the polish picks up the residual quadratic interaction that the seed ignores.
...
of magnitude cheaper than its closest non-deterministic competitor. The mechanism on
real data is structural, not regime-driven (\S\ref{sec:slh-regime}): the rank-one factor
dominance of $\hat\Sigma$ makes the spectral seed nearly recover the oracle, and the
polish closes the small residual quadratic-interaction gap. This is unrelated to the
Regime~III recovery story that the proposition above describes for synthetic instances.
